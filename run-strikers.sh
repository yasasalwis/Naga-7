#!/bin/bash
################################################################################
# N7-Strikers Launcher
# Called by start.sh to run the Strikers agent in its own terminal window.
...
# Resolve python
if   [ -f "$SCRIPT_DIR/n7-strikers/.venv/bin/python" ]; then
    PYTHON="$SCRIPT_DIR/n7-strikers/.venv/bin/python"
...
LOG_LEVEL="${LOG_LEVEL:-INFO}" exec $PYTHON main.py 2>&1 | tee "$LOG_DIR/n7-strikers.log"
